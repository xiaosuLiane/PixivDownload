...
word = quote(word)
print(word)
cookie = input('[如果有限制級的需要cookie](登錄賬號之後F12找到cookie複製)輸入Cookie:')
header = {
    'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/119.0.0.0 Safari/537.36 Edg/119.0.0.0',
...
while True:
    ...
    if(content.__contains__('error\":true')):
        ...
        break
    p = re.compile('https:\\\/\\\/i.pximg.net\\\/c\\\/250x250_80_a2\\\\/(?:custom-thumb|img-master)\\\\/img(.*?)(?:square1200|custom1200).jpg\",')
    m = p.findall(content)
    print(m)
    for i in m:
